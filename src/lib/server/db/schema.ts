...
export const chatMessages = pgTable("chat_messages", {
  ...
  attachedFile: jsonb("attached_file"), // stores file metadata as JSON
...
export const ragDocumentsRelations = relations(
  ...
  ({ one, many }) => ({
    user: one(users, {
      fields: [ragDocuments.userId],
      references: [users.id],
    }),
    chunks: many(ragDocumentChunks),
  })
);

export const ragDocumentChunksRelations = relations(
  ragDocumentChunks,
  ({ one, many }) => ({
    document: one(ragDocuments, {
      fields: [ragDocumentChunks.documentId],
      references: [ragDocuments.id],
    }),
    embeddings: many(ragDocumentEmbeddings),
  })
);

export const ragDocumentEmbeddingsRelations = relations(
  ragDocumentEmbeddings,
  ({ one }) => ({
    chunk: one(ragDocumentChunks, {
      fields: [ragDocumentEmbeddings.chunkId],
      references: [ragDocumentChunks.id],
    }),
  })
);
...
export type RagDocumentEmbedding = typeof ragDocumentEmbeddings.$inferSelect;
export type NewRagDocumentEmbedding = typeof ragDocumentEmbeddings.$inferInsert;
