<script lang="ts">
  ...
  export let messageIndex: number = 0;
  ...
  const handleRegenerate = async () => {
    if (message.role === "assistant") {
      try {
        // For assistant messages, regenerate the current assistant message
  ...
  const handleEdit = () => {
    // Check if this is the first user message (index 0 and role is user)
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      // Don't show error to user, just cancel edit
      showEditForm = false;
      editedContent = message.content;
    }
  };
    ...
    <div class="max-w-[80%] min-w-0 {message.role === 'user' ? 'ml-auto' : ''}">
      ...
      <div class="prose prose-invert max-w-none rounded-2xl px-4 py-3 {message.role === 'user' ? 'bg-blue-600/20 border border-blue-500/30' : 'bg-gray-700/40 border border-gray-600/60'}">
        {#if showEditForm && message.role === 'user'}
          <div class="space-y-3">
            <textarea
              ...
              class="w-full p-3 bg-gray-800 border border-gray-600 rounded-lg text-white resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 message-edit-textarea"
              rows="3"
            ...
            <div class="flex gap-2">
              <button
                ...
                class="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors text-sm disabled:opacity-60 disabled:cursor-not-allowed"
        ...
        {:else}
          ...
          {:else}
            <EnhancedMessageRenderer content={message.content} isStreaming={false} citations={citations} />
          {/if}
        {/if}
      </div>
      ...
      {#if message.attachedFile}
        <div class="mt-3">
          <span class="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-blue-500/10 border border-blue-500/30 text-blue-300 text-sm">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <span class="font-medium">{message.attachedFile.name}</span>
      ...
      <div class="mt-2 flex items-center justify-center gap-3 text-xs text-gray-300">
        ...
        <span>
          {(versions.findIndex(v => v.isActive) + 1) || 1} / {Math.max(versions.length, 1)}
        </span>
        <button
        ...
        {/if}
            ...
            {#if message.role === 'assistant'}
              ...
              <button
                ...
                class="p-2 text-gray-400 hover:text-white hover:bg-gray-700/50 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-gray-800 rounded-lg p-6 max-w-md mx-4 border border-gray-600">
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 class="text-lg font-semibold text-white">Cannot Edit First Message</h3>
            <p class="text-sm text-gray-400">First message cannot be forked</p>
          </div>
        </div>
        <p class="text-gray-300 mb-6">
          The first message in a conversation cannot be edited or forked. This helps maintain the conversation structure and prevents issues with the chat flow.
        </p>
        <div class="flex justify-end">
          <button
            on:click={() => showFirstMessagePopup = false}
