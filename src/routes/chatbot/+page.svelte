<script lang="ts">
	...
	const scrollToBottom = () => {
		if (scrollAreaRef && scrollAreaRef.querySelector) {
    ...
    // Load conversations on mount and restore last opened chat
    onMount(async () => {
        await chatStore.loadConversations();
        
        // Check URL parameters for new chat
        ...
        if (newChat === 'true') {
            ...
            chatStore.startNewConversation();
            // Clean up URL
            replaceState('/chatbot', {});
        } else {
            // Check if there's a current chat ID in localStorage
            try {
                const lastChatId = localStorage.getItem('currentChatId');
                if (lastChatId) {
                    // Try to load the last chat, but don't show error if it doesn't exist
                    await chatStore.loadMessages(lastChatId);
                }
            } catch (error) {
                // If loading last chat fails, just start fresh
                console.log('No previous chat to restore, starting fresh');
            }
        }
        
        // Ensure scroll to bottom after page load
        setTimeout(() => {
            scrollToBottom();
        }, 200);
    });
...
<div class="h-screen flex bg-black text-white">
	...
	<div class="flex-1 flex flex-col relative z-0">
		...
		<div class="absolute top-4 {sidebarOpen ? 'left-4' : 'left-20'} z-50 model-dropdown transition-all duration-300">
			<div class="relative">
				...
				{#if showModelDropdown}
					<div class="absolute top-full left-0 mt-1 w-48 bg-black border border-gray-600 rounded-lg shadow-2xl max-h-60 overflow-y-auto z-50 model-dropdown-scroll">
		...
		<ScrollArea bind:this={scrollAreaRef} class="flex-1 bg-gradient-to-br from-gray-900 via-black to-gray-900 relative overflow-hidden chat-scroll-area">
