...
export const POST: RequestHandler = async ({ locals, params, request }) => {
  try {
    ...
    // Include conversation context up to the edited message (excluding the edited message itself)
    const editedTime = new Date(
      editedUserMessage.timestamp as unknown as string
    );
    const contextUpToEdit = conversationContext.filter(
      (msg) => new Date(msg.timestamp as unknown as string) < editedTime
    );

    console.log("Generating AI response for edited message:", content.trim());
    let aiResponse = "";
    try {
      aiResponse = await aiService.generateContextualResponse(
        content.trim(),
        contextUpToEdit.map((m) => ({
      ...
      console.log("AI response generated:", aiResponse ? "Success" : "Empty");
    } catch (aiError) {
      console.error("AI service error:", aiError);
      // Continue with empty response, will use fallback
    }

    // Fallback to non-contextual generation if response is empty
    if (!aiResponse || !aiResponse.trim()) {
      try {
        console.log("Trying fallback AI generation");
        aiResponse = await aiService.generateResponse(
          content.trim(),
          "User",
          "gemini-2.5-pro"
        );
        console.log("Fallback AI response:", aiResponse ? "Success" : "Empty");
      } catch (e) {
        console.error("Fallback AI generation failed:", e);
        aiResponse =
